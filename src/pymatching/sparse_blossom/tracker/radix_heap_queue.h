...
struct radix_heap_queue {
    ...
    FloodCheckEvent dequeue() {
        ...
        if (bit_buckets[0].empty()) {
            ...
            size_t b = 1;
            while (b < bit_buckets.size() && bit_buckets[b].empty()) {
                b++;
            }
            if (b >= bit_buckets.size()) {
                _num_enqueued = 0;
                throw std::invalid_argument(
                    "radix_heap_queue invariant violation: _num_enqueued is nonzero but all buckets are empty");
            }
