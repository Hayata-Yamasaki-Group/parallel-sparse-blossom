...
extern thread_local std::string graph_flooder_debug_context;
...
struct GraphFlooder {
    ...
    /// Returns true iff there is at least one scheduled tentative event in the queue.
    /// This is a queue-level predicate and may still count stale deferred events.
    bool has_scheduled_tentative_events() const;
    /// Returns true iff there is at least one desired tentative event that would survive queue validation.
    bool has_valid_tentative_events() const;
    /// Returns the earliest scheduled tentative event time currently in the queue.
    /// This is intended for correctness-first scheduling and may observe stale deferred events.
    cumulative_time_int peek_next_scheduled_time() const;
    /// Returns the earliest valid tentative event time implied by desired queue trackers.
    /// Unlike peek_next_scheduled_time(), this ignores stale deferred events and reflects the next
    /// event time that dequeue_valid() could eventually produce.
    cumulative_time_int peek_next_valid_tentative_event_time() const;
    /// Returns true iff one of the next valid tentative events at the earliest valid time would
    /// claim ownership of a watched detector node. The watched mask is indexed by local node id.
    bool next_valid_event_would_claim_any_watched_detector(const std::vector<uint8_t>& watched_mask) const;
    /// Returns the detector node ids that would be claimed by the next valid tentative event time.
    std::vector<size_t> next_valid_event_claimed_detector_nodes() const;
    /// Processes exactly one valid tentative event and returns the resulting MWPM notification,
    /// or NO_EVENT when the tentative event caused only flooder-local state updates.
    MwpmEvent step_one_tentative_event_returning_mwpm_event();
