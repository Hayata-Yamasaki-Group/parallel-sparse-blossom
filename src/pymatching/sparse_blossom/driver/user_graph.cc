...
const pm::MatchingGraph& pm::UserGraph::get_matching_graph_for_parallel_clustering() {
    return get_mwpm().flooder.graph;
}

pm::ProcessingClusterGraphCache* pm::UserGraph::get_processing_cluster_graph_cache() {
    if (!_processing_cluster_graph_cache) {
        _processing_cluster_graph_cache = std::make_unique<pm::ProcessingClusterGraphCache>(
            pm::build_processing_cluster_graph_cache(get_matching_graph_for_parallel_clustering()));
    }
    return _processing_cluster_graph_cache.get();
}
