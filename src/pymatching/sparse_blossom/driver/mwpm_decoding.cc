...
void pm::process_timeline_until_completion(pm::Mwpm& mwpm, const std::vector<uint64_t>& detection_events) {
    ...
    initialize_mwpm_for_detection_events(mwpm, detection_events);
...
void pm::initialize_mwpm_for_detection_events(pm::Mwpm& mwpm, const std::vector<uint64_t>& detection_events) {
    if (!mwpm.flooder.queue.empty()) {
        throw std::invalid_argument("!mwpm.flooder.queue.empty()");
    }
...
pm::MatchingResult pm::extract_compact_result_from_current_mwpm_state(
    pm::Mwpm& mwpm, const std::vector<uint64_t>& detection_events) {
    if (!mwpm.is_quiescent_without_shatter()) {
        throw std::invalid_argument("MWPM must be quiescent before extracting a compact matching result.");
    }
    size_t num_observables = mwpm.flooder.graph.num_observables;
    if (num_observables > sizeof(pm::obs_int) * 8) {
        throw std::invalid_argument("compact matching result extraction requires at most 64 observables.");
    }
    ...
    bit_packed_res.weight += mwpm.flooder.negative_weight_sum;
    return bit_packed_res;
}

pm::ExtendedMatchingResult pm::extract_result_from_current_mwpm_state(
    pm::Mwpm& mwpm, const std::vector<uint64_t>& detection_events) {
    if (!mwpm.is_quiescent_without_shatter()) {
        throw std::invalid_argument("MWPM must be quiescent before extracting a stopped-state result.");
    }
    ...
    if (num_observables > sizeof(pm::obs_int) * 8) {
        ...
        mwpm.extract_paths_from_match_edges(mwpm.flooder.match_edges, result.obs_crossed.data(), result.weight);
        for (auto& obs : mwpm.flooder.negative_weight_observables) {
            result.obs_crossed[obs] ^= 1;
        }
        result.weight += mwpm.flooder.negative_weight_sum;
    } else {
        pm::MatchingResult bit_packed_res =
            shatter_blossoms_for_all_detection_events_and_extract_obs_mask_and_weight(mwpm, detection_events);
        if (!mwpm.flooder.negative_weight_detection_events.empty()) {
        ...
        bit_packed_res.obs_mask ^= mwpm.flooder.negative_weight_obs_mask;
        fill_bit_vector_from_obs_mask(bit_packed_res.obs_mask, result.obs_crossed.data(), num_observables);
        result.weight = bit_packed_res.weight + mwpm.flooder.negative_weight_sum;
    }

    return result;
}

pm::ExtendedMatchingResult pm::decode_detection_events_in_user_graph(
    pm::UserGraph& graph,
    const std::vector<uint64_t>& detection_events,
    const DecoderConfig& config) {
    if (config.mode == DecoderMode::PARALLEL_SPARSE_BLOSSOM) {
        if (config.edge_correlations) {
            throw std::invalid_argument(
                "parallel sparse blossom does not yet support edge-correlated decoding.");
        }
        pm::LockstepSchedulerConfig scheduler_config;
        scheduler_config.edge_correlations = false;
        scheduler_config.cluster_config.beta = config.parallel_config.beta;
        scheduler_config.cluster_config.gamma = config.parallel_config.gamma;
        scheduler_config.cluster_config.lambda = config.parallel_config.lambda;
        scheduler_config.cluster_config.max_level = config.parallel_config.max_level;
        scheduler_config.allow_global_fallback =
            config.parallel_config.runtime_mode == ParallelDecoderRuntimeMode::PRODUCTION_SAFE;
        scheduler_config.num_workers = config.parallel_config.num_workers;
        auto result = pm::lockstep_hierarchical_decode(graph, detection_events, scheduler_config);
        return result.root_aggregate_result;
    }

    auto& mwpm = config.edge_correlations ? graph.get_mwpm_with_search_graph() : graph.get_mwpm();
    ExtendedMatchingResult result(graph.get_num_observables());
    decode_detection_events(mwpm, detection_events, result.obs_crossed.data(), result.weight, config.edge_correlations);
    return result;
}
